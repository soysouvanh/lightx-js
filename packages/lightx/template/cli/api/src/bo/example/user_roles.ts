/**
 * Business Object - User_roles CRUD
 * Exhaustive pedagogical implementation.
 */
import { User_rolesDao } from "../../dao/example/user_roles.dao.js";
...
export async function read(payload: any, contexts: any) {
  ...
  const rows = await exe.query("SELECT * FROM `user_roles` LIMIT 1");
...
export async function list(payload: any, contexts: any) {
  ...
  const rows = await exe.query("SELECT * FROM `user_roles` LIMIT ? OFFSET ?", [limit, offset]);
...
export async function create(payload: any, contexts: any) {
  const exe = contexts.getExamplePool();
  // Using DAO insert
  const data = await User_rolesDao.insert(exe, payload);
