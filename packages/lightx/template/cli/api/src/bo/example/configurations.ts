/**
 * Business Object - Configurations CRUD
 * Exhaustive pedagogical implementation.
 */
import { ConfigurationsDao } from "../../dao/example/configurations.dao.js";

// Read Method
export async function read(payload: any, contexts: any) {
  const exe = contexts.getExamplePool();
  const data = await ConfigurationsDao.findById(exe, payload.id || payload.code || payload.raw_id);
...
export async function list(payload: any, contexts: any) {
  ...
  const data = await ConfigurationsDao.listByOffset(exe, offset, limit);
...
export async function create(payload: any, contexts: any) {
  const exe = contexts.getExamplePool();
  // Using DAO insert
  const data = await ConfigurationsDao.insert(exe, payload);
...
export async function update(payload: any, contexts: any) {
  const exe = contexts.getExamplePool();
  await ConfigurationsDao.updateById(exe, payload.id || payload.code || payload.raw_id, payload.patch || payload);
...
export async function remove(payload: any, contexts: any) {
  const exe = contexts.getExamplePool();
  await ConfigurationsDao.deleteById(exe, payload.id || payload.code || payload.raw_id);
