/**
 * Business Object - Order_items CRUD
 * Exhaustive pedagogical implementation.
 */
import { Order_itemsDao } from "../../dao/example/order_items.dao.js";
...
export async function read(payload: any, contexts: any) {
  ...
  const rows = await exe.query("SELECT * FROM `order_items` LIMIT 1");
...
export async function list(payload: any, contexts: any) {
  ...
  const rows = await exe.query("SELECT * FROM `order_items` LIMIT ? OFFSET ?", [limit, offset]);
  return { status: 200, data: rows, pagination: { limit, offset } };
}
...
export async function create(payload: any, contexts: any) {
  const exe = contexts.getExamplePool();
  // Using DAO insert
  const data = await Order_itemsDao.insert(exe, payload);
