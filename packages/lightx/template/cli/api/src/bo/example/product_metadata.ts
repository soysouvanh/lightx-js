/**
 * Business Object - Product_metadata CRUD
 * Exhaustive pedagogical implementation.
 */
import { Product_metadataDao } from "../../dao/example/product_metadata.dao.js";

// Read Method
export async function read(payload: any, contexts: any) {
  const exe = contexts.getExamplePool();
  const data = await Product_metadataDao.findById(exe, payload.id || payload.code || payload.raw_id);
...
export async function list(payload: any, contexts: any) {
  ...
  const data = await Product_metadataDao.listByOffset(exe, offset, limit);
...
export async function create(payload: any, contexts: any) {
  const exe = contexts.getExamplePool();
  // Using DAO insert
  const data = await Product_metadataDao.insert(exe, payload);
...
export async function update(payload: any, contexts: any) {
  const exe = contexts.getExamplePool();
  await Product_metadataDao.updateById(exe, payload.id || payload.code || payload.raw_id, payload.patch || payload);
...
export async function remove(payload: any, contexts: any) {
  const exe = contexts.getExamplePool();
  await Product_metadataDao.deleteById(exe, payload.id || payload.code || payload.raw_id);
