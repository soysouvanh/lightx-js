...
import type { GenericExecutor } from './executor.js';

/**
 * =========================================================================
 * DAOX NATIVE DRIVER INTEGRATIONS
 * =========================================================================
 * Implements the GenericExecutor boundary for each major SQL dialect.
 * Maps internal streaming abstractions (O(1) memory) directly to native C/C++ Node.js binaries.
 * Zero data cloning, zero serialization - pure driver redirection.
 */
...
export class PostgresExecutor implements GenericExecutor {
  ...
  async query<T>(sql: string, params?: unknown[]): Promise<T[]> {
    return (await this.sql.unsafe(sql, params as never[])) as unknown as T[];
  }

  async *stream<T>(sql: string, params?: unknown[]): AsyncIterable<T> {
    const cursor = this.sql.unsafe(sql, params as never[]).cursor(100);
    for await (const chunk of cursor) {
      for (const row of (chunk as unknown[])) {
        yield row as T;
      }
    }
  }
...
export class SqlServerExecutor implements GenericExecutor {
  ...
  private prepare(req: MsSqlRequest, params?: unknown[]): MsSqlRequest {
    if (params) {
      for (let i = 0; i < params.length; i++) {
        req.input('p' + String(i + 1), params[i]);
      }
    }
    return req;
  }
