...
export function buildDaoTests(dialect: string, table: TableSchema, importPath: string = '.'): string {
  ...
  code += `      await ${entity}Dao.insert(mockExecutor, { _dummy: 1 } as any);\n`;
  ...
  if (pkName) {
    ...
    code += `      await ${entity}Dao.updateBy${PkTitle}(mockExecutor, 1 as any, { _dummy: 1 } as any);\n`;
    code += `      expect(mockExecutor.query).toHaveBeenCalled();\n`;
    code += `    });\n\n`;
  ...
  code += `      await ${entity}Dao.insertBatch(mockExecutor, [{ _dummy: 1 } as any]);\n`;
  code += `      expect(mockExecutor.query).toHaveBeenCalled();\n`;
  code += `    });\n`;
  code += `  });\n`;
