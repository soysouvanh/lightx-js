...
export async function scanPostgres(url) {
  ...
  `;
    ...
    for (const c of columnsResult) {
        ...
        if (!arr)
            colsByTable.set(c.table_oid, (arr = []));
        arr.push(c);
    }
    ...
    for (const pk of pkeysResult) {
        ...
        if (!arr)
            pksByTable.set(pk.table_oid, (arr = []));
        arr.push(pk);
    }
    ...
    for (const ix of indexesResult) {
        ...
        if (!arr)
            idxsByTable.set(ix.table_oid, (arr = []));
        arr.push(ix);
    }
