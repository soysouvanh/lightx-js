...
export async function scanOracle(url) {
  ...
  `);
    ...
    WHERE owner = sys_context('userenv','current_user')
  `);
    const pksResult = await conn.execute(`
  ...
  `);
    const idxsResult = await conn.execute(`
